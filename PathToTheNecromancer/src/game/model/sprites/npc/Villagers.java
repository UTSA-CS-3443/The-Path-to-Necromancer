...
public class Villagers extends CharacterSprites implements InteractionSprites {
	...
	private DialogueGraph getRandomDialogue() {
		...
		case 0:
			graph.addNode(
					"I do admit, my little boy rides a horse like he was born on one, crying, naked, and confused");
			break;
		case 1:
			graph.addNode(
					"I must admit, after the necromancer's general came in, everyone has been free of sickness, free of pain!");
			break;
		case 2:
			graph.addNode(
					"Thought my kid was a picky eater until I saw him try only the most exotic of foods, like dirt, and some green stuff in the corner of my house");
			break;
		case 3:
			graph.addNode(
					"While I do like the fact the necromancer's general is coming in everyday to cure us, I hate the fact he(or she?) is so...weird...");
			break;
		case 4:
		...
		case 5:
			graph.addNode(
					"I have to admit, the sounds of the necromancer's skeleton army is slightly less offputting thanks to the xylophone sound the skeletons make as they walk");
			break;
		case 6:
			graph.addNode(
					"If there are infinite universes, what if we're in one where there only exists one universe?");
			break;
		}
		return graph;
	}
	...
	private DialogueGraph getSetDialogue() {
		DialogueGraph graph = new DialogueGraph();
		graph.addNode("Welcome to out humble village traveler. *cough,cough*"); // 0
		graph.addNode("what happened here?"); // 1
		graph.addNode("I mean a common cold."); // 2
		graph.addNode(
				"Oh, it's a sad story, the necromancer used the hippie to siphon the life from this land and our people to the other side of the mountain."); // 3
		graph.addNode("That's terrible!"); // 4
		graph.addNode(
				"Indeed, but we have lived here for years and shall continue to do so, why are you here traveler?"); // 5
		graph.addNode("I have come here to help you!"); // 6
		graph.addNode("I have come to slay the necromancer!"); // 7
		graph.addNode("I came for some of your famous mushroom soup"); // 8
		graph.addNode(
				"Ah, I am sorry traveler the life has been taken from our mushrooms as well and now all that is left is poison inside of them."); // 9
		graph.addNode("Nooooooo! well I came for..."); // 10
		graph.addNode("Wait how are you even alive, if the mushroom are poisonous?"); // 11
		graph.addNode("Well, you see... We survive on the flesh of mortals, GET HIM LADS!"); // 12
		graph.addNode("Ah how wonderful, if you can slay the necromancer we will finally be free!"); // 13
		graph.addNode("What do you mean, aren’t you free right now?"); // 14
		graph.addNode(
				"Alas traveler, we built a maze to the south to prevent monsters from attacking when our food grew short, and the path west is too dangerous."); // 15
		graph.addNode("Well I just freed the west side."); // 16
		graph.addNode("Don’t worry I will free you!"); // 17
		graph.addNode("How wonderful, thank you traveler!"); // 18
 		graph.addNode("Ah then you wouldn’t mind if we ask you for some food?"); // 19
		graph.addNode("Sure why not?"); // 20
		graph.addNode("I am all out of food, but how about after I kill the Necromancer I treat you to a meal?"); // 21
		graph.addNode("*drooling* thank you ma boy, GET HIM LADS"); // 22
		graph.addNode(
				"That would make the meal taste even better, sure why not, we can wait a while longer. We have waited this long after all."); // 23
		graph.addNode("Trust me you don't want to know."); // 24
		graph.addNode("No, son me and this village have been sick a long time and we need help."); // 25
		graph.addNode("What do you mean sick?"); // 26
		graph.addNode("Is there a cure?"); // 27
		graph.addNode("That’s a shame."); // 28
		graph.addNode(
				"There is no cure for us son, the only way to help us is to defeat the necromancer, only then may we be free."); // 29
		graph.addNode("What do you mean?"); // 30
		graph.addNode("I mean our bodies and the land aren’t the only thing affected by the Necromancer my boy."); // 31
		graph.addNode("Huh?"); // 32
		graph.addNode(
				"Our minds have been taken over by the curse. It turned us into evil creatures and now we hunger for flesh."); // 33
		graph.addNode("That’s horrible!"); // 34
		graph.addNode("Are you gonna eat me?"); // 35
		graph.addNode("How can I save you?"); // 36
		graph.addNode("Like I said kill the Necromancer and we will be eternally grateful"); // 37
		graph.addNode("Is that an invitation?"); // 38
		graph.addNode("Yes"); // 39
		graph.addNode("GET HIM LADS"); // 40
		graph.addNode("No"); // 41
		graph.addNode("Well then, get going before we can’t help ourselves."); // 42
		graph.addNode("Ay, just give the necromancer a good punch for me and let us pass on."); // 43
		graph.addNode("I will."); // 44
